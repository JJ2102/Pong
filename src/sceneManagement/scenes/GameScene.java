...
public class GameScene extends Scene {
    ...
    // Renderer
    private final Renderer renderer; // rechnet die 3D-Objekte auf die 2D-Zeichenfläche um
    private final Camera camera; // Blickpunkt, aus dem die Szene gezeichnet wird

    // Objekte
    private final Player player; // Paddle des Spielers (folgt der Maus)
    private final Enemy aiPlayer; // Paddle der KI
    private final Box box; // Der Spielraum bzw. die Wände drumherum
    private final Ball ball;

    private final SevenSegmentDisplay scoreDisplay; // Punkteanzeige, die als 3D-Objekt an der Seitenwand hängt

    // Hitboxes
    private final BoxHitbox goalPlayerHitbox; // Torfläche vor dem Spieler, Treffer = Punkt für die KI
    private final BoxHitbox goalAIHitbox; // Torfläche hinter der KI, Treffer = Punkt für den Spieler

    // Hit Cooldown
    private final Cooldown hitCooldown; // Sperrzeit nach einem Paddle-Treffer

    // Score
    private int playerScore = 0;
    private int aiScore = 0;
    private enum PlayerType { PLAYER, AI } // unterscheidet, für wen ein Punkt gezählt wird
    private final int winningScore = 4; // Punkte, die zum Sieg nötig sind

    // Scoring
    private final Countdown countdown; // Countdown vor jeder Runde
    private enum GameState { PLAYING, COUNTING_DOWN } // läuft das Spiel gerade, oder wartet es auf den Rundenstart?
    private GameState gameState = GameState.COUNTING_DOWN; // Das Spiel beginnt immer mit einem Countdown

    // Positionen
    private final double boxDepth = 1.5; // halbe Tiefe des Spielfelds (von der Mitte bis zu einer Wand)
    private final double playerPositionZ = -boxDepth + 0.2; // Z-Ebene, auf der sich das Spieler-Paddle bewegt (knapp vor der Wand)
    private Vector3 mousePosition = new Vector3(0, 0, playerPositionZ); // Aktuelle Mausposition im 3D-Raum
    ...
    public GameScene(GameWindow window) {
        ...
        camera.setPosition(new Vector3(0, 0, cameraPositionZ)); // Kamera leicht hinter der vorderen Box-Wand platzieren
    ...
    public void update() {
        if (window.isDebug()) calculateFPS(); // FPS nur mitzählen, wenn sie auch angezeigt werden
    ...
    // Friert das Spiel ein (z.B. beim Öffnen des Pausemenüs): Game-Loop und Countdown anhalten
    @Override
    public void onPause() {
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            window.toggleOverlay(OverlayType.PAUSE); // Pausemenü
        }
